...
@section('content-head')
    <div class="menu-content-head">
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ route('account-manager.school-management') }}">School Management</a></li>
                <li class="breadcrumb-item"><a href="{{ route('account-manager.school-management.view', ['school' => $school->id]) }}">School
                        #{{ $school->id }}</a></li>
...
@section('content-main')
    <main class="menu-content-body">
        ...
        <div class="bg-white shadow-lg w-100 rounded p-4">
            ...
            <form action="{{ route('account-manager.school-management.edit', ['school' => $school->id]) }}" method="post">
                @csrf
                <div class="mt-4">
                    ...
                    <div class="form-floating">
                        <input type="text" class="form-control   @error('name') is-invalid @enderror" id="name"
                            name="name" placeholder="Type school's name" value="{{ old('name') ?? $school->name }}"
                            required>
                    ...
                    <div class="form-floating mt-2">
                        <input type="email" class="form-control @error('email') is-invalid @enderror" id="email"
                            name="email" placeholder="Type school's email" value="{{ old('email') ?? $school->email }}"
                            required>
                    ...
                    <div class="form-floating">
                        <input type="text" class="form-control @error('address_line_1') is-invalid @enderror"
                            ...
                            value="{{ old('address_line_1') ?? $school->address_line_1 }}" required>
                    ...
                    <div class="form-floating mt-2">
                        <input type="text" class="form-control @error('address_line_2') is-invalid @enderror"
                            ...
                            value="{{ old('address_line_2') ?? $school->address_line_2 }}" required>
                    ...
                    <div class="form-floating mt-2">
                        <input type="text" class="form-control @error('address_line_3') is-invalid @enderror"
                            ...
                            value="{{ old('address_line_3') ?? $school->address_line_3 }}">
                    ...
                    <div class="form-floating mt-2">
                        <select class="form-select @error('city') is-invalid @enderror" id="city" name="city"
                            ...
                            @foreach ($cities as $city)
                                <option value="{{ $city->id }}" @if ((old('city') ?? $school->city_id) == $city->id) selected @endif>
                                    {{ $city->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div id="mac-addresses" class="d-flex flex-column" style="max-width: 425px">
                        @php $i = 0; @endphp
                        @foreach ((array) (old('mac_addresses') ?? ($school->RFIDDetectors->toArray() ?? [''])) as $mac)
                            <div class="d-flex w-100 mac-addr">
                                <div class="mt-2 mr-2 flex-grow-1">
                                    <input type="tel"
                                        class="form-control @error('mac_addresses.' . $i) is-invalid @enderror"
                                        name="mac_addresses[]" placeholder="MAC Adresses"
                                        value="{{ old('mac_addresses.' . $i) ?? $mac['mac_address'] }}" required>
                                </div>
                                <div class="ms-2 pt-2">
