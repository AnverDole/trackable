...
@include('user-system-admin.school-management.select-admin')
...
@section('content-main')
        ...
        <div class="container">
            <div class="row" style="max-width: 720px;width:100%">
                <div class="bg-white shadow-lg w-100 rounded p-4">
                    ...
                    <div class="container">
                        <a href="{{ route('school-management') }}" class="mt-2 col-md btn btn-light me-2"><i
                                class="fa-solid fa-circle-arrow-left"></i>&nbsp;Back</a>
                        <a href="{{ route('school-management.edit', ['school' => $school->id]) }}"
                            class="mt-2 col-md btn btn-light  me-2"><i class="fa-solid fa-pen"></i>&nbsp;Edit</a>
                        <a href="{{ route('school-management.students', ['school' => $school->id]) }}"
                            class="mt-2 col-md btn btn-light  me-2"></i>&nbsp;View Students</a>
                        <a href="{{ route('school-management.view.attendances', ['school' => $school->id]) }}"
                            class="mt-2 col-md btn btn-primary"></i>&nbsp;View Attendance</a>
                    </div>
                </div>
                                ...
                                <a @hasRole('super-admin') href="{{ route('admin-management.view', ['admin' => $AssociatedAccountManager->id]) }}"  @endhasRole
                                    ...
                                    <div class="col-12 col-md-1 ol-md-4 d-flex justify-content-end" onclick="return false">
                                        <button type="button" class="btn btn-danger"
                                            admin-id="{{ $AssociatedAccountManager->id }}"
                                            admin-name="{{ $AssociatedAccountManager->firstname . ' ' . $AssociatedAccountManager->lastname }}"
            ...
            <div class="modal-content">
                <form action="{{ route('school-management.admin.dissociate', ['school' => $school->id]) }}" method="post">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-light" data-bs-target="#remove-admin"
    ...
    @yield('select-admin-model')
@endsection

@push('scripts')
    <script>
        let removeAdminModelContainer = $("#remove-admin");
        let removeAdminModel = new bootstrap.Modal(document.getElementById('remove-admin'), {});

        function confirmDeletePrompt(e) {
            let admin_name = $(e).attr("admin-name");
            let admin_id = $(e).attr("admin-id");

            removeAdminModelContainer.find(".admin-name").text(admin_name);
            removeAdminModelContainer.find("input[name=admin_id]").val(admin_id);
            removeAdminModel.show();
        }
    </script>
@endpush
