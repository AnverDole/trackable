@section('select-school-model')
                        ...
                        <form id="search-school-form" onsubmit="return searchSchools()">
                            ...
                            <div class="d-flex justify-content-end mt-3  pb-4">
                                <button type="button" onclick="clearSearch()"
                                    class="btn btn-light me-2"><i class="fa-solid fa-rotate-left"></i>&nbsp;Reset</button>
    ...
    <div class="modal fade" id="confirm-associate-new-school" tabindex="-1"
        ...
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                <form action="{{ route('admin-management.school.associate', ['admin' => $admin->id]) }}" method="post">
                    @csrf
...
@push('scripts')
    <script>
        ...
        function searchSchools() {
            ...
            axios.post("{{ route('school-management.select-school') }}", {
                    name: schoolName.val(),
        ...
        function searchResultHtml(school) {
            let html = `<div class="s-school bg-white text-dark text-decoration-none border row rounded p-2 mt-3 align-items-center">
                    ...
                    <div class="col-12 col-md-3 d-flex flex-column">
                        <button class="btn btn-light select" school-name="${school.name}" school-id="${school.id}"><i class="fas fa-plus-square"></i>&nbsp;Select</button>
                    </div>
                </div>`;
        ...
        $(document).on("click", "#search-results .s-school .select", function() {
            ...
            confirmNewAssociationModelContainer.find(".school-name").text(name);
            confirmNewAssociationModelContainer.find("input[name=school_id]").val(id);
